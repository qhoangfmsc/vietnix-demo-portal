        ...
        {detailTabs.map((tab) => (
          <Button
            ...
            className={`relative flex flex-col h-full overflow-hidden items-center 
              border border-gray-200 rounded-lg p-4 transition-all duration-150 
              cursor-pointer bg-white hover:border-blue-400 
              ${detail === tab.value ? "border-blue-500 shadow-md" : "border-gray-200"} 
          ...
          >
            ...
            <div className="font-semibold text-base text-gray-700 text-pretty">{tab.label}</div>
          </Button>
        ))}
      </div>
      {detail &&
